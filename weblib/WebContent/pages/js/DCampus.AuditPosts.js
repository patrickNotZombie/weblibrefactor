...
$j(function(){
	...
	**/
	if(window.location.href.lastIndexOf("/p/") != -1) {
	   ...
	   DCampus.AuditPostsUI.pageNum = parseInt(locationNum);
	   DCampus.AuditPostsUI.isPaging = true;
	} 			
	$j(".container002").DCampusPaging({
		 "limit":DCampus.AuditPostsUI.limit             /** 显示条数默认20条    **/
		,"start":DCampus.AuditPostsUI.start              /** 开始位置 默认从0开始**/
		,"liststep":DCampus.AuditPostsUI.liststep          /** 分页显示长度 默认10 **/	
		,"pageNum":DCampus.AuditPostsUI.pageNum            /** 默认第一页开始      **/	
		,"pageCount":DCampus.AuditPostsUI.pageCount          /** 页数                **/
		,"totalCount":DCampus.AuditPostsUI.totalCount         /** 列表总数            **/
		,"listbegin":DCampus.AuditPostsUI.listbegin
		,"listend":DCampus.AuditPostsUI.listend			
		,"RequestPath" : System.site+"/post/getAuditPosts.action?temp="+Math.random()
		,"RequestData" : "groupId="+DCampus.AuditPostsUI.mygroupId
		,"PageJson":auditPostsJSON
		,"isPaging"  : DCampus.AuditPostsUI.isPaging
	   ...
	   $j(".btn-pass").bind("click",function(){
			...
			if(confirm("是否确认通过？")) {
			DCampus.Public.AjaxRequestFunction(System.site+"/post/passAuditPost.action",checkedQuery,"POST",DCampus.AuditPostsData._requestPassSuccess);
			}
	   });
	   $j(".btn-not-pass").bind("click",function(){
		   ...
		   if(confirm("是否确认不通过？")) {
		   DCampus.Public.AjaxRequestFunction(System.site+"/post/unpassAuditPost.action",checkedQuery,"POST",DCampus.AuditPostsData._requestPassSuccess)
		   }
	   });	
	   
	  
})
DCampus.AuditPostsUI = {
	limit : 20,//列表条数
	start : 0,//开始值
	liststep : 10,	
	pageNum : 0,	
