...
<script src="../../../jquery-1.4.2.pack.js"></script>
<script src="../../../DCampus.Public.js"></script>
...
$j(function(){
	...
	up = new FancyUpload2($('demo-status'), $('demo-list'), { // options object
		...
		onSelect : function(){
		   $j(".uploadText").hide();
		   if(up.size / 1024 >= <%=restSize%>) {
		     alert("文件超过空间容量，请重新选择文件");
		     up.remove();
			 setTimeout(function(){$j(".uploadText").show();},500)
		   } else {
		     up.start(); // start upload
		   }
		   return false;	   
  ...
  <!-- See index.html -->
    <form action="<%=site%>/group/uploadResource.action" method="post" enctype="multipart/form-data" id="form-demo">
      ...
      <div id="demo-status" class="hide">
       ...
       <span class="uploadText">请选择要上传资源 <br/>单文件上传大小：<span id="sizelimit"></span></span>
      </ul>
      </div>
    </form>
</div>
