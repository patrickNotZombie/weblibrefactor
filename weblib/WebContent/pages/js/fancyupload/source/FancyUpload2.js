...
var FancyUpload2 = new Class({
	...
	options: {
		...
		listRemoveFunction: "" //hjz修改，增加一个删除单个列表后触发的事件
	},

	initialize: function(status, list, options) {//初始化
		this.status = $(status);
		this.list = $(list);
...
FancyUpload2.File = new Class({
	...
	render: function() {
		...
		this.element = new Element('li', {'class': 'file'}).adopt(
			...
			new Element('a', {
				...
				events: {
					click: function() {
						this.remove();
						//hjz修改
						if(typeof(this.base.options.listRemoveFunction) == "function") {
						   this.base.options.listRemoveFunction();
						}
						return false;
					}.bind(this)
				}
			...
			this.info
		).inject(this.base.list);//注入到目标元素内
	},
	...
	onComplete: function() {
		...
		this.base.currentText.set('html', '上传完毕');
...
(function() {
	var phrases = {
		'progressOverall': '总体进度 ({total})',
		'currentTitle': '文件进度',
		'currentFile': '正在上传 "{name}"',
		'currentProgress': '已上传：{bytesLoaded} 传输速度：{rate}, 剩余时间：{timeRemaining} .',
		'fileName': '{name}',
		'remove': '删除',
		'removeTitle': 'Click to remove this entry.',
		'fileError': '上传错误',
		'validationErrors': {
			'duplicate': '文件 <em>{name}</em> 已经在上传队列中,不允许重复上传.',
			'sizeLimitMin': 'File <em>{name}</em> (<em>{size}</em>) is too small, the minimal file size is {fileSizeMin}.',
			'sizeLimitMax': '文件 <em>{name}</em> (<em>{size}</em>) 太大了, 最大只允许 <em>{fileSizeMax}</em>.',
