  ...
  $.fn.DCampusPaging = function(options) {
	    if(typeof(options) != "string") {
				...
				if(settings.PageJson != null && !settings.isPaging) {
					...
					if(settings.RequestPath != null && settings.RequestData != null) {
				      ...
				      switch(reflash) {
						 ...
						 case "Res-reflash" : settings.RequestData = "parentId=" + jQuery("#parentID").val();break;
					  }
			...
			requestThreadsListSuccess = function(jsonObj){
					...
					jQuery(".pageNumA").empty();
					for (var i = settings.listbegin; i < settings.listend; i++) {
					   var current = (i == settings.pageNum ? "class='currentPage'" : "");
					   jQuery(".pageNumA").append("<a "+current+" href='javascript:void(0)'>"+i+"</a> ")
					}		
					jQuery(".pageNumA a").bind("click",function(){
					   pageScrollFunction(jQuery(this).text());
					});
					jQuery(".myThreadPagenum").text(settings.pageNum);
					jQuery(".myThreadpageCount").text(Math.ceil(settings.pageCount));	
					//webmail下拉分页
					 var __html = ""
					 ...
					 for(i=0;i<Math.ceil(countPage);i++) {
						...
						jQuery(".pageSelect").html(__html);
					 }
					 
					 //if(settings.reflash != "next" || settings.reflash != "back") {
					if(typeof(settings.reflash) != "string") {
						setTimeout(function(){jQuery(".pageSelect option").attr("selected","");},1)					
						$.each(jQuery(".pageSelect"),function(n,i){
							setTimeout(function(){jQuery(".pageSelect").eq(n).find("option").eq(settings.pageNum - 1).attr("selected","selected");},1)							
						});
					}
			}
    ...
    /**
		    分页事件
		**/
		jQuery(".nextPage").unbind().bind("click",function(){
			if (settings.pageNum < settings.pageCount) {
			   settings.pageNum = settings.pageNum + 1;
			   pageScrollFunction(settings.pageNum);
			}		
		});
		 jQuery(".prePage").unbind().bind("click",function(){
			  settings.pageNum = settings.pageNum - 1;
			  pageScrollFunction(settings.pageNum);
		 });	
		 jQuery(".go_botton").unbind().bind("click",function(){
			pageScrollFunction(jQuery(this).parent().parent().parent().find(".pageInput").val());
			jQuery(".pageInput").val("");
		 });	
		 jQuery(".pageInput").unbind().bind("keypress",function(event){
			  if(event.keyCode == 13) {
          ...
          //webmail下拉分页
		  jQuery(".pageSelect").unbind().bind("change",function(){
